...
{
	/**
	 *	@~english @brief Temporary rvalues likely to be stored in registers.
	 *	@~russian @brief Временные rvalue значения, скорее всего, находящиеся в регистрах.
	 */
	RVALUE_KIND_TEMP,
	/**
	 *	@~english @brief Immediate rvalues of three types: integers, floatings, strings.
	 *	@~russian @brief 'Константные' rvalue трех типов: целые, вещественные, строки.
	 */
	RVALUE_KIND_IMM,
	/**
	 *	@~english @brief Generic rvalues that are not generated by IR, but still could be used during generation.
	 *	@~russian @brief Общие rvalue значения, не гененрируются IR, но могут использоваться при генерации.
	 */
	RVALUE_KIND_GENERIC
} rvalue_kind;
...
/**
 *	@~english
 *	@brief Data about extern, it is passed to gen functions.
 *
 *	@~russian
 *	@brief Информация о внешних функциях, передается в функции генерации.
 */
typedef struct exetrn_data
{
	item_t id;
	item_t type;
} extern_data;
...
/**
 *	@~english
 *	@brief Data about global variables, it is passed to gen functions.
 *
 *	@~russian
 *	@brief Информация о глобальных переменных, передается в функции генерации.
 */
...
/**
 *	@~english
 *	@brief Data about functions, it is passed to gen functions.
 *
 *	@~russian
 *	@brief Информация о функциях, передается в функции генерации.
 */
...
typedef void (*ir_gen_extern_func)(void *const enc, const extern_data *const data);
/** Format: type name.*/
typedef void (*ir_gen_global_func)(void *const enc, const global_data *const data);
/** Format: type name().*/
typedef void (*ir_gen_function_func)(void *const enc, const function_data *const data);
/** Format: -*/
